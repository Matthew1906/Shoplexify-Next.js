...
import { roboto_light, roboto_semibold } from "@/app/_lib/font";
import { Product } from "@/app/_lib/interface"
import { currencyString, popularityString, trimString } from "@/app/_lib/string";
...
const ProductCard = ({product}:{product:Product})=>{
    return <Link href={`/products/${product.slug}`}>
        <div className="w-full h-full p-4 shadow-lg shadow-neutral-400">
            ...
            />
            <b className={`block mt-2 ${roboto_light.className} text-lg`}>{trimString(product.name, 15)}</b>
